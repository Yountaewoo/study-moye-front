...
import { updatePost, fetchPostDetail, type UpdatePostRequest, type PostDetailResponse } from "@/lib/api"
...
export default function EditStudyPage() {
  ...
  useEffect(() => {
    ...
    const loadPost = async () => {
      ...
      if (result.success && result.data) {
        ...
        if (locationId) {
          const parents = getParentLocations()
          const parent = parents.find(p => {
            const children = getChildLocations(p.id)
            return children.some(c => c.id === locationId)
          })
          
          if (parent) {
            locationParent = String(parent.id)
            locationChild = String(locationId)
          } else {
            // 부모 지역인 경우
            locationParent = String(locationId)
          }
        }
        ...
        setFormData({
          ...
          isOnline: data.locationId === null ? "true" : "false",
          locationParent: locationParent,
          locationId: locationChild || locationParent,
          minNumber: String(data.minNumber),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updatePost(postId, requestData)
      
      if (result.success) {
        alert("스터디가 수정되었습니다!")
        router.push(`/posts/${postId}`)
      } else {
        alert(result.error || "스터디 수정에 실패했습니다.")
      }
    } catch {
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
          <Link href={`/posts/${postId}`} className="flex items-center gap-2 text-muted-foreground hover:text-foreground">
      ...
      <main className="mx-auto max-w-2xl px-4 py-8">
        <Card className="border-border bg-card">
          <CardHeader>
            <CardTitle className="text-2xl text-foreground">스터디 수정</CardTitle>
